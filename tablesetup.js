...
const createTables = (req, res, next) => {
  if (req.params.authorizationCode == config.app.accessKeyId) {
    ...
    table.addColumn(new Column('parent_id', Column.Type.ID, null));
    table.addColumn(new Column('repost_id', Column.Type.ID, null));
    ...
    res.status(401).send({'error':'Missing/wrong authorization code'});
  }
};
